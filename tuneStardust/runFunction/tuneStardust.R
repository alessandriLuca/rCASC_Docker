...
#' @param spot_coordinates, a character string indicating the path of the spot_coordinates file, with file name and extension included
...
#'}
#' @export
tuneStardust <- function(group=c("sudo","docker"), scratch.folder, file,spot_coordinates,separator){
...
system(paste("cp ",spot_coordinates," ",scrat_tmp.folder,sep=""))

  #executing the docker job
    params <- paste("--cidfile ",data.folder,"/dockerID -v /var/run/docker.sock:/var/run/docker.sock -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d --privileged=true repbioinfo/tunestardust Rscript /home/main.R ",basename(file)," ",basename(spot_coordinates)," ",separator," ",scrat_tmp.folder,sep="")
