...
p <- add_argument(p, "tissuePositions", help="matrix separator ")
...
tissuePositions=argv$tissuePositions
...
tissuePositionsM=as.matrix(read.table(tissuePositions,sep=separator,header=TRUE,row.names=1))
rownames(tissuePositionsM)=gsub("-",".",rownames(tissuePositionsM))
...
f=data.frame(x=as.numeric(tissuePositionsM[colnames(mainMatrix),c(2)]),y=as.numeric(tissuePositionsM[colnames(mainMatrix),c(3)]))
