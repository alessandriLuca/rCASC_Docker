...
if np.asarray(sys.argv).shape[0] ==1:
    ...
    projectName="prova"
    matrix="setA_interest_SAVER.csv"
    sep=","
    bName="setA_interest_SAVER_clustering.output.csv"
    lrVar=0.01
    beta_1Var=0.9
    beta_2Var=0.999
    epsilonVar=1e-08
...
os.system("Rscript /home/plot.R")
ccc=cm.rainbow(np.linspace(0,1,minFeature))
X_tsne = TSNE(learning_rate=200).fit_transform(DFexample)
plt.clf()
