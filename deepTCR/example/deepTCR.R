...
#' @param folderFiles, Path of folder with all the files folder
...
deepTCR <- function(group=c("sudo","docker"),scratch.folder,folderFiles,resFolderCustom="NULL"){

fastqPath=folderFiles
genomeFolder=fastqPath
      dockerImage="repbioinfo/deeptcr:v1"
...
#storing the position of the home folder
  ...
  params <- paste("--cidfile ",genomeFolder,"/dockerID  -v ", scrat_tmp.folder, ":/scratch -d ",dockerImage, " python3 /home/main.py ",basename(genomeFolder),sep="")
...
system(paste("cp -r ",genomeFolder," ",scrat_tmp.folder,sep=""))
  ...
  res=paste(genomeFolder,"Results",sep="/")}
    ...
    dir.create(res)
    system(paste("cp -R ", scrat_tmp.folder, "/*_Results* ",res, sep=""))
